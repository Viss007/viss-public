...
import {
  getApiKey,
  getComposioOnboardingBaseUrl,
  getHost,
  getPort,
  getPublicUrl,
  isProvider,
  Provider,
  buildComposioConnectUrl,
  buildComposioOnboardingUrl,
  getComposioOauthAuthorizeUrl,
  getComposioOauthClientId,
  getComposioOauthRegisterUrl,
  getComposioOauthTokenUrl
} from "./config.js";
...
function normalizeMcpBody(rawBody: unknown): unknown {
...
function activeOauthClientId(): string {
  return runtimeOauthClientId || getComposioOauthClientId();
}

function renderOauthPage(title: string, message: string, tone: "success" | "error"): string {
...
app.use((req, _res, next) => {
  (req as ReqWithId).reqId = randomUUID().slice(0, 8);
  next();
});

app.get("/v1/health", (_req, res) => {
  let publicBase: string | null = null;
  let onboardingBaseUrl: string | null = null;
  try {
...
app.get("/v1/onboarding/status", requireApiKey, (req, res) => {
  ...
  if (!userRef) return fail(res, 400, "Missing user_ref");
  if (!isProvider(providerRaw)) return fail(res, 400, "Missing or invalid provider");
  ...
  const hasAnyConnectedProvider = Boolean(
    userState &&
...
app.get("/v1/onboarding/connect_url", requireApiKey, (req, res) => {
  ...
  const callbackState = Buffer.from(
    JSON.stringify({
      user_ref: userRef,
      provider: providerRaw
    }),
...
app.get("/v1/onboarding/callback", (req, res) => {
  ...
  if (!stateRaw) return fail(res, 400, "Missing callback state");
  try {
    ...
    res.status(200).type("text/html").send(
      "<html><body><h1>Connection saved</h1><p>You can close this tab and return to ChatGPT.</p></body></html>"
    );
  } catch {
...
app.get("/v1/oauth/composio/start", (req, res) => {
  ...
  if (!clientId) {
    return fail(
      res,
      400,
      "Missing OAuth client. Register runtime client first via POST /v1/oauth/composio/register_client."
    );
  }
...
app.get("/v1/oauth/composio/launch", (req, res) => {
  const renderLaunchError = (message: string): string => `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
...
app.post("/mcp", async (req, res) => {
  ...
  if (!authHeader) return fail(res, 401, "Missing Authorization header");
...
app.get("/v1/oauth/composio/runtime_token_status", requireApiKey, (req, res) => {
  ...
  const expiresInSeconds =
    runtimeToken?.expiresAt && runtimeToken.expiresAt > now
...
app.get("/v1/oauth/composio/callback", async (req, res) => {
  ...
  if (!state) return fail(res, 400, "Missing state");
  const session = runtimeOauthSessions.get(state);
  if (!session) return fail(res, 400, "Invalid or expired state");
...
app.post("/v1/tools/execute", requireApiKey, (req, res) => {
  ...
  if (!userRef) return fail(res, 400, "Missing user_ref");
  if (!isProvider(providerRaw)) return fail(res, 400, "Missing or invalid provider");
  if (!tool) return fail(res, 400, "Missing tool");
