...
type ToolMode = 'public' | 'memory' | 'full';
...
function dimensionalEnabled(): boolean {
  if (toolMode() === 'public') return false;
  const url = process.env.DIMENSIONAL_HTTP_URL?.trim().toLowerCase();
  if (!url || url === 'off' || url === 'disabled' || url === '0') return false;
  return true;
}
...
export function registerAgentTools(server: McpServer): void {
  ...
  if (dimensionalEnabled()) {
    server.registerTool(
      ...
      {
        description:
          'Optional memory HTTP bridge when DIMENSIONAL_HTTP_URL is configured. Disabled on Public :3333 island.',
        inputSchema: {
          action: z
            .string()
            .describe(
              'dimensional action name: health, search, pre_hook, write, stats, reload, ...',
            ),
          params: z
      ...
      async ({ action, params }) => {
        try {
          ...
          return { content: [{ type: 'text' as const, text: `dimensional error: ${msg}` }] };
        }
      },
    );
  }
  ...
  server.registerTool(
    ...
    {
      description:
        'Reads a file from the local filesystem under allowed workspace roots. Lines are numbered as LINE|CONTENT. Optional offset/limit for large files.',
      inputSchema: {
        ...
        offset: z
          ...
          .optional()
          .describe(
            'Line to start from (1-based from start, or negative from end). Omit to read from start.',
          ),
        limit: z
          .number()
          .optional()
  ...
  server.registerTool(
    ...
    {
      description:
        'Search file contents using ripgrep (rg) under allowed roots. Mirrors Cursor Grep parameters.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        'Call an MCP tool from your local Cursor mcp.json (stdio or url servers). Set MCP_CALL_ALLOW_SERVERS=comma list to restrict. Reads CURSOR_MCP_CONFIG or ~/.cursor/mcp.json.',
      inputSchema: {
