...
function buildRgArgs(p: GrepParams, searchPath: string): string[] {
  const args: string[] = [];
...
export async function grepLocal(p: GrepParams): Promise<string> {
  ...
  return await new Promise((resolve) => {
    ...
    child.on('error', (e: NodeJS.ErrnoException) => {
      if (e.code === 'ENOENT') {
        resolve(
          'Error: `rg` (ripgrep) not found on PATH. Install: https://github.com/BurntSushi/ripgrep or `winget install BurntSushi.ripgrep`',
        );
      } else {
    ...
    child.on('close', (code) => {
      ...
      if (head != null && head >= 0) sliced = sliced.slice(0, head);
