...
const transports = {};
function parseProcessLeadBody(body) {
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
        return { ok: false, error: 'Expected a JSON object' };
    }
    const o = body;
    const pick = (k) => (typeof o[k] === 'string' ? o[k].trim() : undefined);
 ...
 */
function normalizeMcpJsonRpcBody(body) {
    if (!body || typeof body !== 'object' || Array.isArray(body))
        return;
    const o = body;
    if (Object.prototype.hasOwnProperty.call(o, 'params'))
        return;
 ...
 */
function injectMcpBridgeHeadersFromBody(req) {
    const extra = {};
    const body = req.body;
    if (!body || typeof body !== 'object' || Array.isArray(body))
        return extra;
...
function patchIncomingMessageForMcpHeaders(req, opts) {
    ...
    if (!needsAcceptOverride && Object.keys(extra).length === 0)
        return;
    const namesToReplace = new Set();
    if (needsAcceptOverride)
        namesToReplace.add('accept');
    for (const k of Object.keys(extra))
        namesToReplace.add(k.toLowerCase());
    ...
    if (Array.isArray(raw)) {
        ...
        const appendCanonical = (lower, value) => {
            const canon = lower === 'mcp-session-id'
                ? 'Mcp-Session-Id'
 ...
 */
function injectSessionMetaIntoInitializeSend(transport, getSessionId) {
...
export function createMcpApp() {
    ...
    app.get('/health', (_req, res) => {
        ...
        res.json({
            ...
            dimensionalHttpUrl: process.env.DIMENSIONAL_HTTP_URL?.trim() || 'http://127.0.0.1:8811/api/dimensional',
    ...
    app.all('/mcp', async (req, res) => {
        try {
            ...
            const extraFromBody = req.method === 'POST' && req.body ? injectMcpBridgeHeadersFromBody(req) : {};
            ...
            if (sessionId && transports[sessionId]) {
                ...
                injectSessionMetaIntoInitializeSend(streamTransport, () => establishedStreamableSessionId || streamTransport.sessionId || undefined);
            }
            else {
                res.status(400).json({
                    jsonrpc: '2.0',
    ...
    app.post('/messages', async (req, res) => {
        ...
        const sessionId = typeof q === 'string' ? q : Array.isArray(q) && typeof q[0] === 'string' ? q[0] : undefined;
...
if (isDirectRun) {
    ...
    app.listen(port, host, () => {
        ...
        console.log(`Streamable HTTP (OpenAI deploy / ngrok): https://<public-host>/mcp  →  http://${loopback}:${port}/mcp`);
        console.log(`Legacy SSE: https://<public-host>/sse`);
        console.log(`Health: http://${loopback}:${port}/health`);
        if (host === '0.0.0.0' || host === '::') {
            console.warn('Security: exposing Read/Grep/CallMcpTool to the internet is dangerous. Use auth, VPN, or MCP_CALL_ALLOW_SERVERS.');
            console.warn('Tunnel tip: use MCP_HOST=0.0.0.0 (not 127.0.0.1) so ngrok Host headers are not rejected by localhost validation.');
        }
    });
}
