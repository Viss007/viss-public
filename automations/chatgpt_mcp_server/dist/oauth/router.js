...
import { clickupAuthorizeUrl, getProviderConfig, getPublicBaseUrl, googleAuthorizeUrl, } from './providers.js';
...
export function createOAuthRouter() {
    ...
    r.get('/auth/:provider', (req, res) => {
        ...
        if (!publicBase) {
            res
                .status(500)
                .type('html')
                .send(htmlPage('Configuration', '<p>Set <code>LEADFLOW_PUBLIC_URL</code> to your public https origin (ngrok).</p>'));
            return;
        }
        ...
        if (!cfg) {
            res
                .status(500)
                .type('html')
                .send(htmlPage('OAuth', `<p>Missing env for <code>${escapeHtml(p)}</code> (client id / secret).</p>`));
            return;
        }
    ...
    r.get('/auth/:provider/callback', async (req, res) => {
        ...
        if (!pending || pending.provider !== p) {
            res
                .status(400)
                .type('html')
                .send(htmlPage('OAuth', `<p>Invalid or expired state. Start again from <code>/auth/${escapeHtml(p)}</code>.</p>`));
            return;
        }
        ...
        res
            .status(200)
            .type('html')
            .send(htmlPage('Connected', '<h1>Connection successful</h1><p>You can close this window and return to ChatGPT.</p>'));
    });
    r.get('/auth/status', (req, res) => {
        ...
        if (!linkRaw) {
            res.status(400).json({
                ok: false,
                error: 'Pass ?link= with the same opaque value you used when opening /auth/google?link=... or /auth/clickup?link=...',
            });
            return;
        }
