...
/**
 * Speed-to-lead automation on Public :3333 (/automations/speed-to-lead/*).
 * Loopback Express+BullMQ child; not desk 9872 agent loop.
 */
class SpeedToLeadProxyController extends Controller
{
    private function internalBase(): string
    {
        return rtrim((string) config('vissai.speed_to_lead_internal_url', 'http://127.0.0.1:3001'), '/');
    }

    private function publicBasePath(): string
    {
        return '/automations/speed-to-lead';
    }
    ...
    public function health(): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if (! is_array($decoded)) {
                return response()->json(['ok' => false, 'error' => 'speed_to_lead_bad_health'], $res->status() ?: 502);
            }
            if ($res->successful()) {
                ...
                $decoded['publicLeadPath'] = $base.'/lead';
                $decoded['publicGoogleAuthPath'] = $base.'/auth/google';
            }
            ...
            return response()->json([
                'ok' => false,
                'error' => 'speed_to_lead_unavailable',
    ...
    public function forward(Request $request, string $internalPath = ''): StreamedResponse|\Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            return response()->json([
                'ok' => false,
                'error' => 'speed_to_lead_proxy_failed',
                'message' => $e->getMessage(),
                'publicBasePath' => $this->publicBasePath(),
            ], 502);
        }
