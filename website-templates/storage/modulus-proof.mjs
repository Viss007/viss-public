...
(async () => {
  ...
  try {
    ...
    const metrics = await page.evaluate(() => {
      const root = document.querySelector('.tpl-modulus');
      ...
      const visible = (el) => {
        ...
        return {
          ...
          height: r.height,
          inview: el.classList.contains('dvele-inview'),
        };
      };
      return {
        ...
        sectionCount: root ? root.querySelectorAll('section').length : 0,
      };
    });
    ...
    const m8124 = await page2.evaluate(() => {
      const hero = document.querySelector('.dvele-hero h1');
      return {
        heroText: hero?.textContent?.trim() || null,
